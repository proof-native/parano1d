//! Research-only composite owner + programmable-policy authorization capsule.
//!
...
//! different bounded programs share the existing 2^11-cell authorization
...
const PROGRAM_BYTES: usize = 16;
const PROGRAM_STEPS: usize = PROGRAM_BYTES;
...
impl Instruction {
    fn encode(self) -> u8 {
        ((self.immediate & 0x3f) << 2) | self.opcode as u8
    }

    fn decode(byte: u8) -> Self {
        let opcode = match byte & 0x03 {
            0 => Opcode::Nop,
...
impl PolicyCase {
    ...
    fn program_word(&self) -> Block128 {
        let bytes = self.program.map(Instruction::encode);
    ...
    fn boundary(&self) -> PolicyBoundary {
        let program = self.program_word();
        PolicyBoundary {
            owner: AuthCapsuleBoundaryPublic::canonical(self.owner_address()),
            policy_initial: [self.current, self.context, program, self.call_nonce],
            policy_final: [self.expected_next(), self.context, program, self.call_nonce],
        }
    }

    fn statement(&self) -> PolicyStatement {
        PolicyStatement {
            program: self.program_word(),
...
struct PolicyBoundary {
    ...
    policy_initial: [Block128; STATE_LANES],
    policy_final: [Block128; STATE_LANES],
}
...
impl PolicyStatement {
    ...
    fn boundary(self) -> PolicyBoundary {
        PolicyBoundary {
            owner: AuthCapsuleBoundaryPublic::canonical(self.owner_address),
            policy_initial: [self.current, self.context, self.program, self.call_nonce],
            policy_final: [self.next, self.context, self.program, self.call_nonce],
        }
    }
}
...
fn policy_statement_digest(
    ...
    sponge.absorb_pair(program, current);
    sponge.absorb_pair(next, context);
    sponge.absorb(call_nonce);
...
fn decode_program(word: Block128) -> [Instruction; PROGRAM_STEPS] {
    word.0.to_le_bytes().map(Instruction::decode)
}
...
fn build_state(case: &PolicyCase) -> Vec<Block128> {
    ...
    let program = case.program_word();
    let mut row = [case.current, case.context, program, case.call_nonce];
    let start = state_index(POLICY_BASE_ROW, 0);
    state[start..start + STATE_LANES].copy_from_slice(&row);
    for (step, instruction) in case.program.iter().enumerate() {
        let immediate = Block128::from(instruction.immediate as u128);
        row[0] = match instruction.opcode {
            Opcode::Nop => row[0],
            Opcode::AddImmediate => row[0] + immediate,
            Opcode::MultiplyImmediate => row[0] * immediate,
            Opcode::SelectContextImmediate => row[0] + row[1] * (row[0] + immediate),
        };
        let start = state_index(POLICY_BASE_ROW + step + 1, 0);
        state[start..start + STATE_LANES].copy_from_slice(&row);
    }
    state
}
...
struct PolicyTables {
    ...
    policy_coefficient: [Vec<Block128>; STATE_LANES],
    policy_cross_01: Vec<Block128>,
    policy_constant: Vec<Block128>,
}
...
impl PolicyPublicTables {
    fn build(program_word: Block128) -> Self {
        ...
        let mut owner_rc: [Vec<Block128>; STATE_LANES] =
            std::array::from_fn(|_| vec![Block128::ZERO; ZK_AUTH_CAPSULE_BANK_LEN]);
        let mut policy_coefficient: [Vec<Block128>; STATE_LANES] =
            std::array::from_fn(|_| vec![Block128::ZERO; ZK_AUTH_CAPSULE_BANK_LEN]);
        let mut policy_cross_01 = vec![Block128::ZERO; ZK_AUTH_CAPSULE_BANK_LEN];
        let mut policy_constant = vec![Block128::ZERO; ZK_AUTH_CAPSULE_BANK_LEN];
        ...
        for index in 0..ZK_AUTH_CAPSULE_BANK_LEN {
            ...
            active[index] = Block128::ONE;
            let instruction = program[row - POLICY_BASE_ROW];
            if output_lane == 0 {
                let immediate = Block128::from(instruction.immediate as u128);
                match instruction.opcode {
                    Opcode::Nop => policy_coefficient[0][index] = Block128::ONE,
                    Opcode::AddImmediate => {
                        policy_coefficient[0][index] = Block128::ONE;
                        policy_constant[index] = immediate;
                    }
                    Opcode::MultiplyImmediate => policy_coefficient[0][index] = immediate,
                    Opcode::SelectContextImmediate => {
                        policy_coefficient[0][index] = Block128::ONE;
                        policy_coefficient[1][index] = immediate;
                        policy_cross_01[index] = Block128::ONE;
                    }
                }
            } else {
                policy_coefficient[output_lane][index] = Block128::ONE;
            }
        }
...
impl PolicyTables {
    fn build(bank: ZkAuthCapsuleBankView<'_>, program_word: Block128) -> Self {
        let public = PolicyPublicTables::build(program_word);
        ...
        Self {
            ...
            increment,
            lane,
            owner_mds: public.owner_mds,
            owner_sigma: public.owner_sigma,
            owner_rc: public.owner_rc,
            policy_coefficient: public.policy_coefficient,
            policy_cross_01: public.policy_cross_01,
            policy_constant: public.policy_constant,
        }
    }

    fn boolean_relation(&self, index: usize) -> Block128 {
        let mut q = self.increment[index] + self.policy_constant[index];
        for lane in 0..STATE_LANES {
            let state = self.lane[lane][index];
            let sigma = self.owner_sigma[lane][index];
            let with_rc = state + self.owner_rc[lane][index];
            let owner_pi = sigma * pow7_base(with_rc) + (Block128::ONE + sigma) * state;
            q += self.owner_mds[lane][index] * owner_pi;
            q += self.policy_coefficient[lane][index] * state;
        }
        q += self.policy_cross_01[index] * self.lane[0][index] * self.lane[1][index];
        self.active[index] * q
    }
...
fn main_round(
    ...
    let active = restrict_high(&tables.active, prior);
    ...
    let owner_mds: [Vec<Block256>; STATE_LANES] =
        std::array::from_fn(|index| restrict_high(&tables.owner_mds[index], prior));
    let owner_sigma: [Vec<Block256>; STATE_LANES] =
        std::array::from_fn(|index| restrict_high(&tables.owner_sigma[index], prior));
    let owner_rc: [Vec<Block256>; STATE_LANES] =
        std::array::from_fn(|index| restrict_high(&tables.owner_rc[index], prior));
    let policy_coefficient: [Vec<Block256>; STATE_LANES] =
        std::array::from_fn(|index| restrict_high(&tables.policy_coefficient[index], prior));
    let policy_cross_01 = restrict_high(&tables.policy_cross_01, prior);
    let policy_constant = restrict_high(&tables.policy_constant, prior);
    let half = 1usize << current_variable;
    assert_eq!(active.len(), 2 * half);

    let mut round = Polynomial::zero();
    for lower_index in 0..half {
        let active = endpoint(&active, lower_index, half);
        let mut q = endpoint(&increment, lower_index, half).add(&endpoint(
            &policy_constant,
            lower_index,
            half,
        ));
        for input_lane in 0..STATE_LANES {
            let state = endpoint(&lane[input_lane], lower_index, half);
            ...
            q.add_assign(&mds.mul(owner_pi));
            q.add_assign(&endpoint(&policy_coefficient[input_lane], lower_index, half).mul(state));
        }
        q.add_assign(
            &endpoint(&policy_cross_01, lower_index, half)
                .mul(endpoint(&lane[0], lower_index, half))
                .mul(endpoint(&lane[1], lower_index, half)),
        );
        let weighted = active.mul(q).scale(boolean_eq_weight(
...
fn terminal_main(
    ...
    let active = evaluate_mle_low_to_high(&tables.active, point).unwrap();
    let mut q =
        operands.increment + evaluate_mle_low_to_high(&tables.policy_constant, point).unwrap();
    for lane in 0..STATE_LANES {
        ...
        q += mds * (sigma * pow7_wide(state + rc) + (Block256::ONE + sigma) * state);
        let coefficient =
            evaluate_mle_low_to_high(&tables.policy_coefficient[lane], point).unwrap();
        q += coefficient * state;
    }
    let cross = evaluate_mle_low_to_high(&tables.policy_cross_01, point).unwrap();
    q += cross * operands.lane[0] * operands.lane[1];
    active * q
}

fn terminal_main_from_public_program(
    program_word: Block128,
    point: &[Block256; ZK_AUTH_CAPSULE_BANK_VARS],
    operands: TerminalOperands,
) -> Block256 {
    let tables = PolicyPublicTables::build(program_word);
    let active = evaluate_mle_low_to_high(&tables.active, point).unwrap();
    let mut q =
        operands.increment + evaluate_mle_low_to_high(&tables.policy_constant, point).unwrap();
    for lane in 0..STATE_LANES {
...
fn build_policy_post_claim_relation(
    ...
    for lane in 0..STATE_LANES {
        let mut source = vec![Block256::ZERO; ZK_AUTH_CAPSULE_BANK_LEN];
        source[state_index(POLICY_BASE_ROW, lane)] = Block256::ONE;
        accumulate(&source, Block256::from(boundary.policy_initial[lane]));
    }
    for lane in 0..STATE_LANES {
        let mut source = vec![Block256::ZERO; ZK_AUTH_CAPSULE_BANK_LEN];
        source[state_index(POLICY_BASE_ROW + PROGRAM_STEPS, lane)] = Block256::ONE;
        accumulate(&source, Block256::from(boundary.policy_final[lane]));
    }
...
fn validate_policy_boundary(
    ...
    for lane in 0..STATE_LANES {
        let initial = bank.state()[state_index(POLICY_BASE_ROW, lane)];
        if initial != boundary.policy_initial[lane] {
            return Err(format!("policy initial boundary mismatch at lane {lane}"));
        }
        let final_state = bank.state()[state_index(POLICY_BASE_ROW + PROGRAM_STEPS, lane)];
        if final_state != boundary.policy_final[lane] {
            return Err(format!("policy final boundary mismatch at lane {lane}"));
        }
    }
    Ok(())
}
...
fn prove_policy_owner(
    ...
    let tables = PolicyTables::build(bank, statement.program);
    ...
    let public_main_final =
        terminal_main_from_public_program(statement.program, &terminal_point, terminal_operands);
...
fn run_full_pcs_case(case: &PolicyCase) -> Value {
    ...
    json!({
        "name": case.name,
        "samples": SAMPLES,
        "program_word_hex": format!("{:032x}", statement.program.0),
...
fn run_case(case: &PolicyCase) -> Value {
    ...
    let tables = PolicyTables::build(bank_view, case.program_word());
    ...
    let mut negative_checks = 0;
    let mut wrong_program = case.program;
    wrong_program[0] = Instruction {
        opcode: Opcode::MultiplyImmediate,
        immediate: 61,
    };
    let wrong_tables = PolicyTables::build(
        bank_view,
        Block128::from(u128::from_le_bytes(wrong_program.map(Instruction::encode))),
    );
    assert!(!wrong_tables.validate_boolean_relation());
    negative_checks += 1;
    ...
    let corrupted_tables = PolicyTables::build(corrupted_view, case.program_word());
    assert!(!corrupted_tables.validate_boolean_relation());
    negative_checks += 1;

    let mut wrong_boundary = case.boundary();
    wrong_boundary.policy_final[0] += Block128::ONE;
    ...
    json!({
        "name": case.name,
        "program_word_hex": format!("{:032x}", case.program_word().0),
        ...
        "public_boundary_claims": 4 + 2 * STATE_LANES,
...
fn main() {
    ...
    let result = json!({
        ...
        "kind": "composite_private_owner_plus_programmable_policy_complete_native_pcs_and_wire_not_historystep",
        ...
        "established": [
            ...
            "two distinct 128-bit programs use one fixed transition relation",
            ...
            "current and next State plus context and nonce boundary binding",
            "program, State, context and nonce statement digest sensitivity",
        ...
        "not_established": [
            ...
            "binding the research policy statement to the production Tx8x2 body",
            "contract code larger than 128 bits",
