...
和 `GF(2^128)`。[安全模型](../protocol/security-model.md)给出包含旧祖先链和矩阵退役的 v2 资源评估。
...
合约 State 所有者是公开条款和计数器的承诺。调用证明活跃领取或恢复方的
秘密，区块证明还执行程序和策略。知道秘密不能绕过承诺限制。
见[活权利](proof-native-contracts.md)。
