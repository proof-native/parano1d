...
P2P 端口 `9600` 服务公网对等节点。`9601` 上的 RPC 包含钱包和进程控制方法。
本地 TCP 回环具有所有者权限；远程挖矿和运营者令牌仅开放各自固定的较窄方法集。
合约方法要求本地所有者权限。HTTP Bearer 令牌不加密流量，远程 RPC 需要私有
隧道或受保护的 TLS 传输。
...
两个类别的单区块终端上限均为 **1,100,000 字节**。一次性分叉来源交换有独立的
**48 MiB** 响应上限。它认证从选定旧边界发生的转换；后续 v2 证明保持该绑定，
无需在每个区块附带完整证书。合约公开条款及授权封装随交易意图按各自限制传输。
[文件、端口与限制](../reference/files-and-ports.md)分别列出 RPC、回执和意图上限。
