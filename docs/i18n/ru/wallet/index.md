...
Скриншоты показывают английский интерфейс кошелька в изолированной тестовой
сети v2. Адреса, балансы и высоты относятся к этому локальному тесту.
...
В **F7** три сценария работы с контрактами:
...
- **Мои контракты**: откройте сохранённый контракт, проверьте права активного
  ...
  сохранённые операции, включая вызовы другой стороны и импортированные чеки,
  и позволяет сохранить проверенный чек после подтверждения.
...
Файлы контрактов и чеки операций открываются через **F7 → Открыть файл**.
Импорт дополняет существующий контракт по идентификатору операции, сохраняя
ваши записи и локальное название. Повторный импорт не создаёт дубликат.
Файл контракта содержит не более одного чека, а не весь журнал отправителя;
для других пропущенных операций нужны отдельные чеки. **F4 → Чеки** относится
к обычным платежам. Если кошелёк пропустил вызов, который уже удалён у пиров,
получите свежие условия или чек вызова у участника: они восстановят счётчики,
после чего кошелёк проверит их текущие остатки.

**Лимит комиссии вызова** ограничивает комиссию будущего вызова. Значение
1 NOID по умолчанию не означает плату за создание: сохранение условий без
пополнения бесплатно, а фактическая комиссия пополнения показывается отдельно
...
`contract-activity/` и полный каталог `objects/` вместе с резервной копией кошелька: публичные правила
...
[Подробное руководство контрактов](../contracts/gui.md) содержит весь процесс и английские скриншоты.
